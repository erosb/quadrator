...
class MapBackedResultSet
        implements ResultSet {

    private final Map<String, Object> data;
    private int cursor = -1; // -1: before first, 0: on first row, 1: after last row
    private boolean wasNull = false;

    public MapBackedResultSet(Map<String, Object> data) {
        this.data = data;
    }
    ...
    public boolean next()
        ...
        if (cursor < 0) {
            cursor = 0;
            return true; // Moved to the single row
        }
        cursor = 1;
    ...
    public void close()
            throws SQLException {
        // No-op for a trivial memory-backed implementation
    }
    ...
    // Helper to fetch and track nulls
    private Object get(String columnLabel)
            throws SQLException {
        if (cursor != 0) {
            throw new SQLException("ResultSet cursor is not on a valid row.");
        }
        // Case-insensitive lookup is common in DBs, but standard map lookup here:
    ...
    // --- Core Getter Implementations ---

    @Override
    public String getString(String columnLabel)
            throws SQLException {
        Object val = get(columnLabel);
    ...
    // --- Stubbing out everything else to satisfy the interface ---
    // (Throwing UnsupportedOperationException for everything else)
